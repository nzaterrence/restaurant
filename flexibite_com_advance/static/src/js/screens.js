            ...
            this.click_product_handler = function(e){
                ...
                if(product){
                    ...
                    if($(e.target).attr('class') === "product-qty-low" || $(e.target).attr('class') === "product-qty"){
                    	...
                    	console.log("product.product_variant_count",product.product_variant_count);
    ...
    screens.ProductCategoriesWidget.include({
    	init: function(parent, options){
            ...
            var records = rpc.query({
                ...
                args: [self.pos.config.id, self.pos.product_fields],
            }).fail(function(){
            	self.pos.db.notification('danger',"Connection lost");
            });
            	...
            	if(result && result[0]){
			    	...
			    	if(total_products){
			    		...
			    		function ajax_product_load(){
			    			var ids = last_ids.splice(0, product_limit);
			    			if(count_loop > 0){
			    				$.ajax({
					                type: "GET",
...
//			    				ajax.rpc("/web/dataset/store_data_to_cache", {'cache_data' : JSON.stringify([data]),
//					            	'product_list' : JSON.stringify(self.pos.product_list)}).then(function(data){
//					            		if(data){					            			
//					            			self.pos.db.notification('success',_t('Cache data stored!'));
//					            		}else{
//					            			self.pos.db.notification('danger',_t('Cache data store error!'));
...
//                      }
    	...
    	click_back: function(){
	        var self = this;
	        var order = this.pos.get_order();
	        if((order.get_credit_detail() && order.get_credit_detail()[0]) || order.get_order_id()){
        ...
        validate_order: function(force_validation) {
            ...
            if (this.order_is_valid(force_validation)) {
    	    	...
    	    	if(order.get_discount_product_id() && order.get_order_total_discount() > 0){
                	...
                	var discount_product_id = order.get_discount_product_id()
                	var current_product_list = self.pos.product_list
                    for(var i = 0, len = current_product_list.length; i < len; i++){
                        if(current_product_list[i].id == discount_product_id){
                           product.display_name = current_product_list[i].display_name
                           break;
                        }
                     }
    ...
    screens.ProductScreenWidget.include({
    	...
    	start: function(){
			...
			self.custom_switch_category_handler = function(event){
				var id = $(event.target).parent().attr('data-category-id') || 0;
...
//            var partner = Number($('.client-line.highlight').attr('data-id'));
	...
	var kitchenScreenWidget = screens.ScreenWidget.extend({
        ...
        init: function(parent, options){
            ...
            if(_.contains(['cook','manager'], cashier.user_role)){
            	rpc.query({
                    model: 'res.users',
	...
	var VariantListWidget = PosBaseWidget.extend({
        ...
        renderElement: function() {
            ...
            for(var i = 0, len = this.filter_variant_list.length; i < len; i++){
            	if (this.filter_variant_list[i] === undefined){
            		continue;
            	}
