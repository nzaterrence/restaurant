...
//                    new Model('aspl.gift.card').get_func('search_read')([['card_no', '=', code], ['expire_date', '>=', today]])
                    .then(function(res){
                        console.info('==RedeemCardPopupWidget==',res);
                        if(res.length > 0){
                            if (res[0]){
        ...
        click_confirm: function(){
            ...
            console.info('==1===', code, self.redeem, self.redeem.card_no);
                            ...
                            if (redeem_amount <= (order.get_due() || order.get_total_with_tax())){
                                ...
                                if( 0 < Number(redeem_amount)){
                                    ...
                                    if(self.redeem && self.redeem.card_value >= Number(redeem_amount) ){
                                        ...
                                        console.info('==5===', order, get_redeem);
                                        //if(get_redeem){
                                            var product = self.pos.db.get_product_by_ids(self.pos.config.enable_journal_id)[0];
                                            console.info('==5.1===', product, self.pos.config.enable_journal_id);
                                            if(self.pos.config.enable_journal_id[0]){
                                               console.info('==5.2===', self.pos.cashregisters);
                                               ...
                                               for ( var j = 0; j < self.pos.cashregisters.length; j++ ) {
                                                ...
                                                console.info('==5.2===', cashregisters);
                                            //}
                                            if (vals){
                                                console.info('==6===');
...
//                $("div#card_invoice").off("click");
    ...
    var RedeemGiftVoucherPopup = PopupWidget.extend({
    	...
    	click_confirm: function(){
	    	...
	    	var self = this;
	    	console.info('===RedeemGiftVoucherPopup===1', self.self_voucher);
	    	var order = self.pos.get_order();
	    	...
	    	if (self.self_voucher){
	    		...
	    		console.info('===RedeemGiftVoucherPopup===2', self.pos.config.gift_voucher_journal_id);
	    		if (self.pos.config.gift_voucher_journal_id.length > 0){
			        for ( var i = 0; i < self.pos.cashregisters.length; i++ ) {

			            console.info('===RedeemGiftVoucherPopup===>',self.pos.cashregisters[i].journal_id,  self.pos.config.gift_voucher_journal_id);

			            if ( self.pos.cashregisters[i].journal_id[0] === self.pos.config.gift_voucher_journal_id[0] ){
			               cashregister = self.pos.cashregisters[i]
			            }
    				}
    				console.info('===RedeemGiftVoucherPopup===3', cashregister,self.pos.cashregisters, self.pos.config.gift_voucher_journal_id);
    				if (cashregister){
    					if(!vouchers){
	...
	var AddToWalletPopup = PopupWidget.extend({
	    ...
	    click_confirm: function(){
	    	...
	    	if(order.get_client()){
	    		...
	    		if(confirm(_t("To add money into wallet you have to select a customer or create a new customer \n Press OK for go to customer screen \n Press Cancel to Discard."))){
            ...
            this.$('.button.close_session').click(function() {
                ...
                rpc.query(params, {async: false}).then(function(res){
                    if(res){
                        ...
                        self.pos.chrome.do_action('flexibite_com_advance.pos_z_report',{
                        	additional_context:{
