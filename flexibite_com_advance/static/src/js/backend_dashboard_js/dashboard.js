    ...
    var POSDashboard = AbstractAction.extend(ControlPanelMixin, {
            ...
            rpc.query({
                ...
                if(res.length > 0){
                    ...
                    if(dataSet.length > 0){
                        ...
                        $(document).find('#top_items_sold').empty();
                        $(document).find('#top_items_sold').append('<div class="alert alert-info"><strong>' + _t('No data found') + '</strong></div>');
                    }
                }else{
                    $(document).find('#top_items_sold').empty();
                    $(document).find('#top_items_sold').append('<div class="alert alert-info"><strong>' + _t('No data found') + '</strong></div>');
            ...
            rpc.query({
                ...
                if(res){
                    ...
                    $(document).find('#journal_pie_chart').append('<div class="alert alert-info"><strong><center>'
                                                                    + _t('No data found') + '</center></strong></div>');
                }
            })
        },
        ...
        chart_journal : function(graph, data){
             AmCharts.makeChart("journal_line_chart", {
              ...
              "theme": "light",
              "titles": [{
                            "text": _t('Month of ') + moment().format('MMMM'),
                            "bold": true
                        }],
              "dataProvider":data,
              "valueAxes":[{
            ...
            rpc.query({
                ...
                if(res){
                    ...
                    if(res['flag'] == true){
                        ...
                        self.$el.find('.journal-option').append('<option data-value="" select="selected">' + _t('Select') + '</option>')
                    ...
                    if(graph_data.length > 0){
                        ...
                        $(document).find('#journal_line_chart').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                    }
                }else{
                    $(document).find('#journal_line_chart').empty();
                    $(document).find('#journal_line_chart').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
            ...
            if(filter_txt === 'year'){
                ...
                rpc.query({
                    ...
                    if(res['final_list'].length > 0){
                        self.dmy_chart(res['final_list'], 'order_month', _t('Months'),  _t("This Year Sale"), _t("Sales : ") + res['currency']+"[[value]]", 'price_total')
                    }else{
                        $(document).find('#chart_day_month_year').empty();
                        $(document).find('#chart_day_month_year').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                    }
                });
            }
                ...
                rpc.query({
                    ...
                    if(res['final_list']){
                        if(res['final_list'].length > 0){
                            self.dmy_chart(res['final_list'], 'days', _t('Days'),  _t("This Month Sale"), _t("Day :") + "[[days]]" + _t(" Sales :") + res['currency']+"[[value]]")
                        }else{
                            $(document).find('#chart_day_month_year').empty();
                            $(document).find('#chart_day_month_year').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                        }
                ...
                rpc.query({
                    ...
                    self.$el.find('#top_hours_hour').append(_t('Amount : ') + result['currency'] + result['top_hour']['amount'] || 0.0)
                    ...
                    if(hrs_pay_data.length > 0){
                        self.dmy_chart(hrs_pay_data, 'hours', _t('Hours'), _t("Today's Hourly Sale"), _t('Sales : ') + result['currency']+' [[value]]')
                    }else{
                        $(document).find('#chart_day_month_year').empty();
                        self.$el.find('#top_hours_hour').empty()
                        self.$el.find('#top_hour_amount').empty()
        ...
        daily_gross_sales: function(company_id){
            ...
            rpc.query({
                ...
                var hrs_pay_data = []
                var today_day = moment().format('dddd, YYYY-MM-DD');
                var last_day = moment().subtract(7, "days").format('dddd, YYYY-MM-DD');
        ...
        onchange_week_option : function(company_id, option){
            ...
            rpc.query({
                ...
                if(res.length > 0){
                    ...
                    $(document).find('#chartweekly').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                }
            });
        },
        ...
        get_top_product_category : function(company_id){
            ...
            rpc.query({
                ...
                if(res['data_source'] && res['data_source'].length > 0){
                    ...
                    $(document).find('#top_product_category').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                }
            });
        },
                        ...
                        if(dataSet.length > 0){
                            $('.employee_time').DataTable({
                                ...
                                columns: [
                                            { title: "" },
                                            { title: _t("Employee") },
                                            { title: _t("Working Hour") },
                                        ]
                               });
                        }else{
                            $(document).find('.employee_time').empty();
                            $(document).find('.employee_time').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                        }
                    }else{
                        $(document).find('.employee_time').empty();
                        $(document).find('.employee_time').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
            ...
            setTimeout(function(){
                ...
                rpc.query({
                    ...
                    if(result){
                        ...
                        self.$el.find('#top_staff_today_amount').append(_t('Amount : ') + result['currency'] + result['top_staff']['amount'])
                        ...
                        if(dataSet.length > 0){
                            ...
                            $(document).find('.top_salesperson').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                        }
                    }else{
                        self.$el.find('#top_staff_today_amount').empty()
                        self.$el.find('#top_staff_today_amount').append('0.0')
                        self.$el.find('#top_staff_today_name').empty()
                        self.$el.find('#top_staff_today_name').append(_t('No Data Found'))
                        $(document).find('.top_salesperson').empty();
                        $(document).find('.top_salesperson').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                    }
                });
            ...
            rpc.query({
                ...
                if(result){
                    ...
                    if(dataSet.length > 0){
                        ...
                        $(document).find('.top-client').append('<div class="alert alert-info"><strong><center>' + _t('No data found') + '</center></strong></div>');
                    }
                }
            });
        },
