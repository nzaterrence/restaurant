...
class PosConfig(models.Model):
    ...
    enable_wechat_pay = fields.Boolean('Wechat Pay')
    enable_alipay = fields.Boolean('AliPay')
    ...
    return_authentication_user_ids = fields.Many2many('res.users','pos_config_return_auth_id',string="Manager Approval")
...
# Take Money Out
    ...
    header_info = fields.Selection([('company','Company'),('store','Shop')],string="Receipt Header",required=True,default="company")
...
class pos_order(models.Model):
    _inherit = 'pos.order'
    transaction_ids = fields.Many2many('payment.transaction', 'res_pos_transaction_rel', 'order_id', 'transaction_id')
    @api.model
...
#             Last 6 month data
    ...
    def _process_order(self,order):
        ...
        _logger.info('old_order_id: {draft_order_id}')
...
#                 return order_id
        ...
        if not order.get('draft_order') and draft_order_id:
            _logger.info('no draft order but have draft order id')
            order_id = draft_order_id
            ...
            if not float_is_zero(order['amount_return'], self.env['decimal.precision'].precision_get('Account')):
                ...
                order_obj.add_payment({
                    ...
                    'payment_name': _('Return Amount'),
                    'journal': cash_journal.id,
                })
            return order_obj
        if not order.get('draft_order') and not draft_order_id:
            _logger.info('have no draft order but have no draft order id')
...
#                 'order_time': date_order.split(" ")[1],
        ...
        for session in pos_session:
            notifications.append([(self._cr.dbname, 'pos.order.line', session.user_id.id), {'screen_display_data': vals}])
...
#             new_order_line.append(order_line)
    ...
    def create(self, values):
        ...
        if values['name'].find('/t') < 0:
            if('pay_ref' in values.keys()):
                target_payment_transaction = self.env['payment.transaction'].search([('reference', '=', values['pay_ref'])])
                transactions = []
                for payment in target_payment_transaction:
                    transactions.append(payment.id)
                order_id.transaction_ids = [(6, 0, transactions)]
...
# POS Reorder end here
    ...
    total_loyalty_earned_points = fields.Float("Earned Loyalty Points", readonly=True, states={'draft': [('readonly', False)]})
    total_loyalty_earned_amount = fields.Float("Earned Loyalty Amount", readonly=True, states={'draft': [('readonly', False)]})
    total_loyalty_redeem_points = fields.Float("Redeemed Loyalty Points", readonly=True, states={'draft': [('readonly', False)]})
    total_loyalty_redeem_amount = fields.Float("Redeemed Loyalty Amount", readonly=True, states={'draft': [('readonly', False)]})
...
#     order_booked = fields.Boolean("Booked", readonly=True)
    ...
    pos_xml_receipt_html = fields.Char(string="Pos Normal Receipt")
...
class pos_order_line(models.Model):
    ...
    @api.model
    def create(self, values):
        modifier_line = values.get('modifier_line', False)
        if modifier_line:
            del values['modifier_line']
        if values.get('combo_ext_line_info', False):
        ...
        if not values.get('back_order'):
            res.create_mo()
        if modifier_line:
            modifier_order_line = self.env['modifier.order.line']
            for modifier in modifier_line:
    ...
    def create_mo(self):
        ...
        """
        if self and self.product_id and self.product_id.product_tmpl_id.id:
...
#         else:
    ...
    floor_id = fields.Many2one(related='table_id.floor_id', string="Table")
...
class pos_session(models.Model):
    ...
    @api.multi
    def get_products_category_data(self,product_fields,flag_config):
        product_list = []
        category_list = []
        ...
        if flag_config == False:
            ...
            domain = ['|',('id', 'in', product_list),('pos_categ_id', 'in', category_list),('sale_ok','=',True),('available_in_pos','=',True)]
        product_records = self.env['product.product'].sudo().search(domain).ids
        ...
        if self:
            ...
            for tax in self.env['account.tax'].browse(tax_list):
                ...
                taxes_info.append({
                    ...
                    'tax_per': "%.2f" % tax.amount,
                    'net_total': "%.2f" % net_total,
                    'gross_tax': "%.2f" % (total_tax + net_total)
                })
        return taxes_info
